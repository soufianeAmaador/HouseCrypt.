...
export class DetailPageComponent implements OnInit {
  ...
  onSubmit(): void{
    ...
    if(parseInt(this.amount.value) > this.property.biddings[0].amount){
      this.property.biddings.unshift(new Bidder("Anon",parseInt(this.amount.value),"0x000000"));
  ...
  //Formvalidations 
  emitChange(): void{
    if(this.amount.value === null || parseInt(this.amount.value) <= this.property.biddings[0].amount) this.isLargestBid = false;
